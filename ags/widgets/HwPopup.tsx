...
function NetworkSpeed() {
    ...
    return (
        <box class="hw-net" spacing={16}>
            <label class="hw-net-label" label={speed((s) => {
...
export default function HwPopup({ gdkmonitor }: { gdkmonitor: Gdk.Monitor }) {
    ...
    const cpu = createPoll("0 0", 3000, [
        "bash", "-c",
        "top -bn1 | grep 'Cpu(s)' | awk '{printf \"%d %d\", $2+$4, NR}'",
    ])
    const cpuPct = createPoll("0", 3000, [
        "bash", "-c",
        "top -bn1 | grep 'Cpu(s)' | awk '{print int($2+$4)}'",
    ])

    const memInfo = createPoll("0 0 0", 5000, [
        "bash", "-c",
        "free -m | awk '/Mem:/{printf \"%d %d %d\", $3, $2, int($3/$2*100)}'",
    ])

    const swapInfo = createPoll("0 0 0", 5000, [
        "bash", "-c",
        "free -m | awk '/Swap:/{printf \"%d %d %d\", $3, $2, ($2>0 ? int($3/$2*100) : 0)}'",
    ])

    const diskInfo = createPoll("0 0 0", 30000, [
        "bash", "-c",
        "df -h / | awk 'NR==2{printf \"%s %s %d\", $3, $2, $5}'",
    ])

    const temp = createPoll("", 5000, [
        "bash", "-c",
        ...
        >
            <box class="hw-popup-content" orientation={Gtk.Orientation.VERTICAL} spacing={12}>
                ...
                <StatRow
                    ...
                    pct={memInfo((m) => parseInt(m.split(" ")[2]) / 100)}
                />
                ...
                <StatRow
                    ...
                    pct={diskInfo((d) => parseInt(d.split(" ")[2]) / 100)}
                />
